...
bool monobit_test() {
    // Test whether the number of zeroes and ones ones in a sequence are approximately the same as
    // would be expected for a truly random sequence. The test assesses the closeness of the
    // fraction of ones to 1/2, that is, the number of ones and zeroes in a sequence should be
    // about the same.
...
bool interval_test() {
    ...
    return chi_square <= expected_chi_square;
}

bool expected_number_test() {
    const int N = (int)1e7;
    ...
    double sum = 0;
    for(int i = 0; i < N; ++i)
        sum += randint(range_left, range_right);

    const double result = sum / N;
    return expected * confidence <= result && result <= expected * (2 - confidence);
}

bool average_distance_test() {
    const int N = (int)3e6;
    ...
    for(int i = 1; i < N; i++)
        std::swap(permutation[i], permutation[randint(0, i)]);

    double distance_sum = 0;
    for(int i = 0; i < N; ++i)
        distance_sum += abs(permutation[i] - i);
    distance_sum /= N;
    return expected * confidence <= distance_sum && distance_sum <= expected * (2 - confidence);
}

void test(std::function<bool()> f) {
...
TEST_CASE("FrequencyTest", "[random]") {
...
TEST_CASE("ExpectationTest", "[random]") {
    SECTION("ExpectedNumber") {
        test(expected_number_test);
    }

    SECTION("AverageDistance") {
        test(average_distance_test);
    }
}